(function () {
    ...
    function touchMove(event) {
        ...
        if (percentage > 0) {
            ...
            slideTypeDict[opt.type].upDrag(percentage, item);

        } else if (item.prev()) {
            //向下拖动
            slideTypeDict[opt.type].downDrag(percentage, item);
        }
    ...
    function nextSlide(item) {
        ...
        if (item.next().length) {
            ...
            delayShowComponent(item.next());
            slideTypeDict[opt.type].nextSlide(item);
        } else {
            slideTypeDict[opt.type].showSlide(item);
        }
    ...
    function prevSlide(item) {
        ...
        if (item.prev().length) {
            ...
            currentItem.attr('state', 'next');
            slideTypeDict[opt.type].prevSlide(item);
        } else {
            slideTypeDict[opt.type].showSlide(item);
        }
    ...
    function initDom(opt) {
        ...
        if (opt.useAnimation) {
            ...
            delayShowComponent(items.first());
        }
    ...
    // 延迟加载单页中的所有带有.part的元素
    function delayShowComponent(dom) {
    ...
    function initEvent(opt) {
        ...
        $('.item').on('transitionend webkitTransitionEnd', function (event) {
            //覆盖层隐藏
            console.log('item transition end')
            $('.overlay').hide();
